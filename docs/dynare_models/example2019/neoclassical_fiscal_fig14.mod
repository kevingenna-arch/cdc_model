...
load('neoclassical_fiscal_fig14_results.mat', 'oo_');
c0=oo_.endo_simul(1,1);
k0 =oo_.endo_simul(2,1);
% g is in oo\_.exo\_simul(:,1) since it is stored in order of declaration
g0 = oo_.exo_simul(1,3)
...
rbig0=1/bet;
rbig=oo_.endo_simul(1,2:101).^(-gam)./(bet*oo_.endo_simul(1,3:102).^(-gam));
nq0=alpha*A*k0^(alpha-1);
nq=alpha*A*oo_.endo_simul(2,1:100).^(alpha-1);
wq0=A*k0^alpha-k0*alpha*A*k0^(alpha-1);
wq=A*oo_.endo_simul(2,1:100).^alpha-oo_.endo_simul(2,1:100).*alpha*A.*oo_.endo_simul(2,1:100).^(alpha-1);
...
subplot(2,3,1)
plot(x,[k0*ones(N,1)],'--k', x,oo_.endo_simul(2,1:N),'k','LineWidth',1.5)
...
subplot(2,3,2)
plot(x,[c0*ones(N,1)],'--k', x,oo_.endo_simul(1,2:N+1),'k','LineWidth',1.5)
title('c','Fontsize',12)
set(gca,'Fontsize',12)
