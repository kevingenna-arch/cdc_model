...
%  The following lines produce a taun sequence with a once and for all jump in taun
% we use shocks to undo that for the first 10 periods and leave taun at
...
k0 = k(1);
n0=n(1);
% taun is in ex_(:,4) since it is stored in alphabetical order
taun0 = oo_.exo_simul(1,4)
...
hold on;
plot(x,[taun0*ones(N,1)],'--k','LineWidth',1.5)
title('\tau_n','Fontsize',12)
axis([0 N -.1 .5])
set(gca,'Fontsize',12)