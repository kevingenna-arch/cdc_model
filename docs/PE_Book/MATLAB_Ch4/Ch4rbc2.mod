...
model;
	...
	% Euler
	exp(ce)^(-sigmaC)*exp(xi)^(1/(1-1/rhoC)-1)*exp(cp)^(-1/rhoC) = beta*exp(ce(+1))^(-sigmaC)*exp(xi(+1))^(1/(1-1/rhoC)-1)*exp(cp(+1))^(-1/rhoC)*(1+exp(r(+1))-delta);
	% hours supply
	exp(w)*exp(ce)^(-sigmaC)*phi*exp(xi)^(1/(1-1/rhoC)-1)*exp(cp)^(-1/rhoC) = psi0*exp(l)^(1/psi1);
