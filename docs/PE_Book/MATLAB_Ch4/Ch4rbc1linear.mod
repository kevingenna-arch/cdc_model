...
model;
    ...
    ce = xi^(1/(1-1/rhoC));
	% Euler
	ce^(gam*(1-sigmaC)-1)*(1-l)^((1-gam)*(1-sigmaC))*xi^(1/(1-1/rhoC)-1)*cp^(-1/rhoC) = beta*ce(+1)^(gam*(1-sigmaC)-1)*(1-l(+1))^((1-gam)*(1-sigmaC))*xi(+1)^(1/(1-1/rhoC)-1)*cp(+1)^(-1/rhoC)*(1+r(+1)-delta);
	% hours supply
	w*gam*ce^(gam*(1-sigmaC)-1)*(1-l)^((1-gam)*(1-sigmaC))*phi*xi^(1/(1-1/rhoC)-1)*cp^(-1/rhoC) = (1-gam)*ce^(gam*(1-sigmaC))*(1-l)^((1-gam)*(1-sigmaC)-1);
