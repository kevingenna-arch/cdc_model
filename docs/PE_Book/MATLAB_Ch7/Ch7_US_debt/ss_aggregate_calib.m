function [ y ] = ss_aggregate_calib(x)
% steady state calibration with endogenous labor supply 
...
%	bigbnew=debtoutputratio*production(kbar,nbar);
	...
	if nr>0
		...
		c(nr+nw) = pen+(1+(1-tauk)*(dbarss-delta))*(k(nw+nr)+b(nw+nr))+trbarss;
	...
	for i=1:1:nage-1
		y(i) = ygrowth^(eta1)*uc(c(i),labors(i))/uc(c(i+1),labors(i+1))-sp(i)*beta1ss*(1+(1-tauk)*(dbarss-delta));
    end	
	...
	transfernew=taxes+bequests+bigbss*((1+popgrowth)*ygrowth-(1+dbarss-delta) )-biggss;
	y(nage+nw+1) = trbarss-transfernew;	
	y(nage+nw+4) = bigbss-debt;
	
	y(nage+nw+5) = rbbar-(1+dbarss-delta);
