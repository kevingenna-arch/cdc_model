...
%{
    ...
    Input:   y : nobs by 1, vector with observations

            mu : scalar, the filter weight (i.e. mu=1600 for quarterly data)

    Output   cy: nobs by 1, vector, the cyclical component of y
...
nobs=size(y,1);
if nobs<=3; error('Input must be an n by 1 vector with n>3'); end;
...
LDAB=KD+1;
INFO=0;
result=lapack('DPBSV',UPLO,nobs,KD,NRHS,m,LDAB,y,nobs,INFO);
